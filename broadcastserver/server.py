...
class Server:
    ...
    def _client_connected_callback(self, reader, writer):
        ...
        logger.debug(
            "Updated dict client_mapping with client id %s", client.id)

        client_task = asyncio.create_task(self.client_handler(client))
        ...
        logger.debug(
            "Updated dict task_mapping with client id %s", client.id)
    ...
    def _cleanup_client_handler_mappings(self, client_id):
        ...
        logger.debug(
            "Deleted client id %s from dict task_mapping", client_id)

        del self.client_mapping[client_id]
        logger.debug(
            "Deleted client id %s from dict client_mapping", client_id)
    ...
    def _cleanup_message_handler(self, task):
        ...
        if (exc is not None) and isinstance(exc, ClientDisconnectedError):
            try:
                ...
                logger.debug(
                    "Cancelling task %s since ClientDisconnectedError was raised",  # noqa: E501
